...
namespace SingularityCore.UI
{
    internal class MenuCollection : IMenuCollection
    {
        ...
        private bool CreateMenu(string parentPath)
        {
            ...
            if ((swCreateCommandGroupErrors) err != swCreateCommandGroupErrors.swCreateCommandGroup_Success) return false;
            ...
            foreach (SingleBaseCommand menuCmd in _commands.Values)
            {
                if (menuCmd.CmdType == CommandType.FlyOut)
                {
                    ...
                    foreach (SingleBaseCommand swFlyOutButton in ((SingleBaseFlyoutCommand)menuCmd).SubCommand)
                        sub._commands.Add(((ISwMenu)swFlyOutButton.Command).MenuOrder, swFlyOutButton);
                    continue; //Now skip stuff below
                }
                if (menuCmd.CmdType != CommandType.Button && menuCmd.CmdType != CommandType.FlyoutSubButton) continue; //Check the cast worked, otherwise skip
                ...
                men.MenuOrder = CmdGroup.AddCommandItem2(menuCmd.Command.CommandName, order, cmd.CommandToolTop, cmd.CommandName, menuCmd.IconIndex, $"CommandCallBack({menuCmd.Id})", $"DisplayStatus({menuCmd.Id})", menuCmd.Id, menutype);
